...
close_btn._alpha = 30;
tick_mc._visible = 0;
_global.styleObj = new TextField.StyleSheet();
_global.styleObj.load("styles.css");
_global.styleObj.onLoad = function(success) {
	if (success) {
		trace("Loadin CSS was sucessful");
	}
};
function fnParseAssessment() {
	...
	for (var i = 0; i<glnFirstChild.length; i++) {
		...
		for (var j = 0; j<glnFirstChild[i].childNodes[2].childNodes.length; j++) {
			...
			eval("option"+(j+1)+"_mc.option"+(j+1)+"_txt").styleSheet = styleObj;
