	...
	for (var i = 0; i<glnFirstChild.length; i++) {
		...
		for (var j = 0; j<=glnFirstChild[i].childNodes[2].childNodes.length-1; j++) {
			...
			if (eval("option"+(j+1)+"_mc.option"+(j+1)+"_txt").maxscroll>1) {
				eval("option"+(j+1)+"_mc.activityScroll")._visible = true;
			} else {
				eval("option"+(j+1)+"_mc.activityScroll")._visible = false;
			}
			...
			if(eval("step"+(j+1)+"_mc.step_"+(j+1)).maxscroll>1)
			{
				eval("step"+(j+1)+"_mc.activityScroll")._visible = true;
			}
			else
			{
				eval("step"+(j+1)+"_mc.activityScroll")._visible = false;
			}
