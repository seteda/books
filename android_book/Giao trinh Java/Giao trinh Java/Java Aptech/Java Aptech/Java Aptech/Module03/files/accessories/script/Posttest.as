...
myData.load("jm_c01_t01/textdata/Posttest.xml");
...
function generateQuestion(sValue) {
	...
	trace(question_Array.length);
	totalcount.text = question_Array.length;
	if (sValue<9) {
		pagecount.text = "0"+(sValue+1);
	} else {
		pagecount.text = (sValue+1);
	}
...
submit_btn.onRelease = function() {
	...
	if (String(activity_Type) == "MCSS") {
		if (String(counter) == String(correct_Array)) {
			...
			if (incorrectCount<10) {
				...
				wrongCount.text = incorrectCount;
			}
		}
	...
	if (quizCounter == question_Array.length-1) {
		...
		_level3.posttest = 2;
		//gotoAndStop(3);
		trace("updating th value of posttest"+_level3.posttest);
	} else {
